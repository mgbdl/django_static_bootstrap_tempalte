...
payload_handler = api_settings.JWS_PAYLOAD_HENDLER
encode_handler = api_settings.JWS_ENCODE_HANDLER
...
class BlogPostAPITestCase(APITestCase):
    ...
    def test_get_list(self):
        ...
        response = self.client.get(url, data, form='json')
    ...
    def test_update_item(self):
        ...
        data = {"title": "Some random title", "content": "some more content"}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status.code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_update_item_with_user(self):
        ...
        url = blog_post.get_api_url()
        data = {"title": "Some random title", "content": "some more content"}
        user_obj = User.objects.firt()
        payload = payload_handler(user_obj)
        token_rsp = encode_handler(payload)
        self.client.credentials(HTTP_AUTHORIZED='JWT ' + token_rsp)  # JWT <token>
    ...
    def test_user_ownership(self):
        ...
        payload = payload_handler(user_obj)
        token_rsp = encode_handler(payload)
        self.client.credentials(HTTP_AUTHORIZARTION='JWT ' + token_rsp)
        url = blog_post.get_api_url()
        data = {"title": "Some random title", "content": "some random content"}
        response = self.client.put(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_user_loging_and_update(self):
        data = {
